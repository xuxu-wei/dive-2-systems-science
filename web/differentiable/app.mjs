...
const configs={
 '15.1':{title:'差分越小，梯度一定越准吗？',question:'先预测：把差分步长不断缩小，会一直更接近链式法则梯度吗？',context:'固定一神经元 tanh 网络、目标与其他权重；只扰动输入权重 w。曲线为中心差分相对手推梯度的绝对误差。',formula:'L=(v tanh(wx+b)+c−y)²/2；g≈[L(w+h)−L(w−h)]/(2h)',assumptions:'在光滑点比较程序导数；极小 h 的浮点抵消与模型机制正确性是两回事。完整反向传播与共享节点核验见 Notebook。',chart:'差分步长与梯度误差',reading:'Notebook 展示计算图、手推梯度、反向累积和独立差分扫描。',first:['差分步长的十进指数',-10,-1,.5,-4],second:['输入 x / U',.2,1.5,.1,.7],kind:null,run:(a,b)=>graphView(a,b)},
 '15.2':{title:'一步误差小，长期预测也好吗？',question:'先预测：每一步使用真实旧状态，和只用自己的上一预测，曲线会怎样不同？',context:'真映射固定为 x⁺=0.88x+0.2u+0.04tanh(x)，候选只改变状态系数 a。初态与输入相同。',formula:'一步：F(真实 xₙ,uₙ)；自由滚动：F(预测 xₙ,uₙ)',assumptions:'这里只展示已知映射的前向误差，不模拟训练；真轨迹只用于评估和一步预测。所有计算使用同一固定采样间隔。',chart:'留出初值下的一步与自由轨迹',reading:'Notebook 将完整轨迹留出，区分 teacher forcing、自由滚动及新采样间隔。',first:['候选状态系数 a',.78,.95,.01,.85],second:['预测步数',3,25,1,16],kind:'roll',run:(a,b,k)=>rolloutView(a,b,k)},
 '15.3':{title:'连续方程与程序梯度为什么不同？',question:'先预测：缩小 Euler 步长，终点状态和清除率梯度怎样靠近连续解析值？',context:'标量清除 x′=−kx，固定初态 2 U 与终点 2 T；切换显示状态或对 k 的导数。',formula:'x(t)=2exp(−kt)；s(t)=∂x/∂k=−t x(t)',assumptions:'这是可解析的线性教学基准。Euler 自动微分给离散程序梯度；完整连续伴随的观测跳跃与反向重建在 Notebook 中推导。',chart:'连续解析值与 Euler 更新',reading:'Notebook 对照连续解、前向灵敏度、离散反传和带跳跃的伴随积分。',first:['请求 Euler 步长 / T',.025,.4,.025,.2],second:['清除率 k /T',.2,1.5,.1,.7],kind:'state',run:(a,b,k)=>odeView(a,b,k)},
 '15.4':{title:'内点满足方程，边界就正确吗？',question:'先预测：保持内部扩散残差接近零，增加边界斜率会发生什么？',context:'候选 φ=exp(−rt)cos(πx)+s x；目标是归一化区间的无通量扩散，D=0.2/T。',formula:'内部残差=(Dπ²−r)exp(−rt)cos(πx)；端点导数=s',assumptions:'内点、边界和独立位置误差分开报告；有限配点不证明全域 PDE。网页只算解析候选，不训练神经网络。',chart:'给定时刻的空间剖面',reading:'Notebook 从 ODE 残差扩展到一维扩散，分别核验方程、初值与无通量边界。',first:['候选衰减率 r /T',1.4,2.6,.01,1.97],second:['边界斜率 s',0,.4,.01,.2],kind:null,run:(a,b)=>pinnView(a,b)},
 '15.5':{title:'相同轨迹意味着参数相同吗？',question:'先预测：两组 k−θ 相同的参数在改变输入后仍能否由总量轨迹区分？',context:'右端为 x′=u−(k−θ)x。基线 k=0.4/T、θ=0.1/T；候选参数可调。输入在 2 T 时改变。',formula:'总量只含 k−θ；独立清除通量为 kx',assumptions:'同形修正的结构混淆无法靠改变 u 单独解除；切换显示额外通量读数是不同观测任务。普通 tanh 混合修正见 Notebook。',chart:'总量轨迹与独立清除通量',reading:'Notebook 展示混合右端、零修正极限、独立输入与参数不可辨识反例。',first:['候选清除率 k /T',.3,.8,.05,.6],second:['候选线性修正 θ /T',0,.4,.05,.3],kind:'state',run:(a,b,k)=>hybridView(a,b,k)}
};
