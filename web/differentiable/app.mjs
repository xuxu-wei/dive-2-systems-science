...
const configs={
 '15.1':{title:'差分越小，梯度一定越准吗？',question:'先预测：把差分步长不断缩小，会一直更接近链式法则梯度吗？',context:'固定一神经元 tanh 网络、目标与其他权重；只扰动输入权重 w。实际测得的量先除以参考量：例如 2 U ÷ 1 U，网络收到无量纲数字2。曲线为中心差分相对手推梯度的绝对误差。',formula:'z=xₚₕᵧₛ/(1 U)；L=(v tanh(wz+b)+c−y)²/2',assumptions:'参数表与 Notebook 中的 x 指网络已接收的无量纲数值 z；物理坐标仍可用 U 标出。极小 h 下，相近数相减可能放大浮点舍入误差。',chart:'差分步长与梯度误差',reading:'Notebook 展示计算图、手推梯度、反向累积和独立差分扫描。',first:['差分步长的十进指数',-10,-1,.5,-4],second:['实际输入 / U',.2,1.5,.1,.7],kind:null,run:(a,b)=>graphView(a,b)},
 '15.2':{title:'一步误差小，长期预测也好吗？',question:'先预测：每一步使用真实旧状态，和只用自己的上一预测，曲线会怎样不同？',context:'状态和输入用 U 记录；送进 tanh 前，先把物理状态除以参考量 1 U，得到无量纲数值。网页代码中的 x 是以 U 计的数值，所以可简写 tanh(x)；候选只改变状态系数 a。',formula:'x⁺=0.88x+0.2u+(0.04 U)tanh(x/(1 U))',assumptions:'滑块直接设定候选系数；一步预测读取真实旧状态，自由运行读取上一步预测。所有计算使用同一固定采样间隔。',chart:'留出初值下的一步与自由轨迹',reading:'Notebook 将完整轨迹留出，区分 teacher forcing、自由滚动及新采样间隔。',first:['候选状态系数 a',.78,.95,.01,.85],second:['预测步数',3,25,1,16],kind:'roll',run:(a,b,k)=>rolloutView(a,b,k)},
 ...
 '15.4':{title:'内点满足方程，边界就正确吗？',question:'先预测：保持扩散方程残差接近零，增加 s·x 后，初值与边界会怎样？',context:'候选 φ=exp(−rt)cos(πx)+s x 表示相对均匀背景的浓度偏差，以 1 U/V 为数值单位；负值表示低于背景，不是负的绝对浓度。目标是归一化区间的无通量扩散，D=0.2/T。加入 s·x 同时改变初始分布与端点条件，比较时同时查看初值差和边界差。',formula:'方程残差=(Dπ²−r)exp(−rt)cos(πx)；初值差=sx；端点导数=s',assumptions:'并列检查方程、初值、边界与参考网格误差。残差和参考误差使用相同的 41 个位置，观察候选与参考解在哪里偏离。',chart:'给定时刻的空间剖面',reading:'Notebook 从 ODE 残差扩展到一维扩散，分别核验方程、初值与无通量边界。',first:['候选衰减率 r /T',1.4,2.6,.01,1.97],second:['边界斜率 s',0,.4,.01,.2],kind:null,run:(a,b)=>pinnView(a,b)},
