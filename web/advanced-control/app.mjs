...
const configs={
 '16.1':{title:'执行上限会改变能量下降吗？',question:'先预测：无饱和的反馈证明对大扰动仍成立吗？',context:'标量非线性系统 x′=−0.4x+0.8x³+u；请求 u=−0.8x−1.2x³，实际输入要截断。先看 V=x²/2 的导数，再切换到短路径。',formula:'V′=x[−0.4x+0.8x³+clip(−0.8x−1.2x³)]',assumptions:'状态 U，时间 T，输入 U/T；路径按当前初值和执行上限计算。',chart:'实际执行与请求反馈',reading:'Notebook 推导未饱和充分条件，逐行核对实际输入和不同初值。',first:['执行上限 / U/T',.2,3,.1,.5],second:['状态或初值 / U',.1,1.5,.1,1.1],kind:'energy',labels:['切换到状态路径','切换回能量导数'],run:feedbackView},
 '16.2':{title:'何时能说整个模型区间稳定？',question:'先预测：只验证两端、使用同一个 P，可以推出哪些中间模型？',context:'二维仿射模型 A(α)=(1−α)A₀+αA₁。P=diag(p,1)，展示 AᵀP+PA 的最大特征值；集合外矩阵作为反例单列。',formula:'P≻0；每个顶点 AᵢᵀP+PAᵢ≺0',assumptions:'结论覆盖 α∈[0,1] 的仿射凸包；外部矩阵不是这两个顶点的凸组合。',chart:'共同二次证书的最大导数特征值',reading:'Notebook 逐项检查顶点、共同 P、凸组合和集合外失效。',first:['区间权重 α',0,1,.05,.5],second:['P 第一对角值',.2,3,.1,1],kind:'inside',labels:['查看集合外反例','返回模型集合'],run:robustView},
 ...
 '16.4':{title:'单条路径能代表随机风险吗？',question:'先预测：重复路径增加、时间步缩小时，模拟均值或方差分别靠近哪条理论曲线？',context:'X 是相对参考水平的稳态偏差，不是非负物质总量；−0.2 U 表示比参考水平低0.2 U。网页依次展示连续方程理论值、当前 Euler–Maruyama 离散规则理论值、有限路径模拟值。',formula:'dX=−aXdt+σdW；Euler：X⁺=(1−ah)X+σ√h Z',assumptions:'a=0 时连续方差为 σ²t。初始偏差为 +1 U。先固定步长增加路径数，观察抽样波动；再固定较多路径缩小步长，比较连续理论与离散理论。',chart:'连续、离散与有限重复三层结果',reading:'Notebook 先固定增量，随后做粗细路径配对和阈值风险实验。',first:['恢复率 a /T',0,1.2,.05,.6],second:['噪声幅度 σ / U/√T',.1,1,.05,.4],kind:'variance',labels:['切换到均值','切换回方差'],run:stochasticView},
 '16.5':{title:'平均守恒就一定达成一致吗？',question:'先预测：相同无向网络在延迟与大步长下，节点会怎样变化？',context:'三节点路径图从[0,4,0]出发。所有更新先读旧历史，再统一写入新状态。',formula:'xₙ₊₁=xₙ−h L xₙ₋d',assumptions:'延迟为 d 步时读取 d 步前的差值；比较前固定初始历史与步数。',chart:'延迟下的局部一致性',reading:'Notebook 推导图平均守恒、无延迟谱界和延迟反例。',first:['同步步长 h / T',.05,.7,.01,.4],second:['整数延迟 / 步',0,3,1,1],kind:'states',labels:['查看节点差距','返回节点轨迹'],run:consensusView},
 '16.6':{title:'训练表变平就已学到最优策略吗？',question:'先预测：重复固定经验如何改变 Q 值，访问计数能说明什么？',context:'同两条经验重复 30 次。终止经验只累加本步奖励，不再使用后继 Q 值。',formula:'Q(s,a)←Q(s,a)+α[r+γmax Q(s′,·)−Q(s,a)]',assumptions:'重复同两条经验，观察已访问动作的更新与未访问信息的差别。',chart:'有限经验下的表格更新',reading:'Notebook 从已知模型的策略评价进入固定经验 Q 更新，区分训练与独立评估。',first:['学习率 α',.05,1,.05,.4],second:['折扣因子 γ',0,.95,.05,.8],kind:'q',labels:['查看动作访问数','返回 Q 值'],run:learningView},
