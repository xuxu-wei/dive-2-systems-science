...
const configs={
 '17.1':{title:'窗口与字典怎样改变复现实验？',question:'先预测：缩短轨迹和缩小候选字典后，哪一项主张仍能由实验核验？',context:'R18 原文 Lorenz 窗口为 100 个归一化时间单位、步长 0.001，并考虑至五次多项式。课程拟合使用前 4 单位、步长 0.01 和二次字典；图展示任务规模差别。',formula:'候选项数 = C(3+d,d)；课程采样点数 = 窗口/0.01 + 1',assumptions:'横轴和读数比较样本点数与字典列数，观察窗口、步长和最高次数的作用。',chart:'课程与原始任务的规模',reading:'Notebook 分别锁定两篇原论文版本、主张、原始任务与课程缩减条件。',first:['课程窗口 / 归一化时间',1,5,.5,4],second:['多项式最高次数',2,5,1,2],kind:'samples',labels:['查看字典列数','返回采样窗口'],run:claimView},
 '17.2':{title:'低训练残差就是方程发现吗？',question:'先预测：改变阈值后，拟合曲线和保留项会怎样变化？',context:'用 x 与 x² 两项回归观察顺序阈值：先拟合，再删除小系数，最后在保留项上重拟合。',formula:'先最小二乘 → 小系数置零 → 保留项重拟合',assumptions:'固定 25 个输入点和同一扰动波形，调节阈值，观察系数与拟合曲线的变化。',chart:'同一小回归的拟合与支持项',reading:'Notebook 给出 Lorenz 稀疏系数、独立初态轨迹与二维连续伴随的有限差分核验。',first:['系数阈值',0,.4,.02,.1],second:['导数噪声代理',0,.5,.05,.1],kind:'fit',labels:['查看保留系数','返回拟合曲线'],run:protocolView},
 '17.3':{title:'失效是否应留在结果表中？',question:'先预测：导数噪声变大，保留项数与无噪参考函数误差一定同时变好吗？',context:'沿用两项回归，固定输入和扰动波形，分别改变扰动强度与稀疏阈值。比较保留项数和相对无噪函数的误差。',formula:'无噪参考误差与支持项数分别报告；失败场景不删除',assumptions:'每次只调一个因素，保留其他设置，以定位误差或支持项改变的原因。',chart:'噪声与结构判定的边界',reading:'Notebook 比较 Lorenz 阈值与噪声，并检查神经动力学候选的步长和结构；配对练习使用给定结果表。',first:['当前噪声强度',0,.5,.1,.2],second:['系数阈值',0,.4,.02,.1],kind:'error',labels:['查看保留项数','返回无噪参考误差'],run:ablationView},
 '17.4':{title:'预测好还需要检查总量收支吗？',question:'先预测：改变双室交换强度，会改变两室总量的收支等式吗？',context:'人工双室清除模型作为生命系统课程迁移。室间交换改变各室路径，但在两个室相加后应抵消。',formula:'d(A₁+A₂)/dt = u₁+u₂−k₁A₁−k₂A₂',assumptions:'状态 U，时间 T；交换系数与一阶消除系数均为 1/T。使用显式 Euler 同步更新两室总量。',chart:'各室状态与总量收支',reading:'Notebook 对同一观测和独立条件比较机制与数据模型，报告误差、收支和失败。',first:['室间交换率 /T',0,.8,.05,.3],second:['室 1 输入 / U/T',0,.6,.05,.2],kind:'amounts',labels:['查看总量收支','返回各室路径'],run:transferView}
};
