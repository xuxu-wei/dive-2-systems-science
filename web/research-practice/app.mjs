...
const configs={
 ...
 '17.3':{title:'失效是否应留在结果表中？',question:'先预测：导数噪声变大，保留项数与无噪参考函数误差一定同时变好吗？',context:'固定小回归的输入和噪声序列，只改变预先声明的噪声强度或稀疏阈值。Neural ODE 的步长与模型消融在 Notebook 另行核对。',formula:'无噪参考误差与支持项数分别报告；失败场景不删除',assumptions:'这里只是网页教学替身，不能称作原文 Lorenz 或 Neural ODE 的消融结果。',chart:'噪声与结构判定的边界',reading:'Notebook 逐项改变稀疏性、导数噪声、求解精度和向量场结构。',first:['当前噪声强度',0,.5,.1,.2],second:['系数阈值',0,.4,.02,.1],kind:'error',labels:['查看保留项数','返回无噪参考误差'],run:ablationView},
 '17.4':{title:'预测好还需要检查总量收支吗？',question:'先预测：改变双室交换强度，会改变两室总量的收支等式吗？',context:'人工双室清除模型作为生命系统课程迁移。室间交换改变各室路径，但在两个室相加后应抵消。',formula:'d(A₁+A₂)/dt = u₁+u₂−k₁A₁−k₂A₂',assumptions:'状态 U，时间 T，交换与清除率 1/T。图中仅为显式 Euler 的共同人工模型，不代表临床有效性。',chart:'各室状态与总量收支',reading:'Notebook 对同一观测和独立条件比较机制与数据模型，报告误差、收支和失败。',first:['室间交换率 /T',0,.8,.05,.3],second:['室 1 输入 / U/T',0,.6,.05,.2],kind:'amounts',labels:['查看总量收支','返回各室路径'],run:transferView}
};
