...
const configs={
 ...
 '14.3':{title:'加入一个平方观测量，会改变预测吗？',question:'先预测：当下一步 y 含 x²，只有 [x,y] 的字典会遗漏什么？',context:'教学三角映射使用 x⁺=0.8x、y⁺=0.7y+0.4x²。先直接删项，再用同一小型训练集分别拟合两项与三项字典。',formula:'[x,y,x²]⁺ = K[x,y,x²]；短字典遗漏 x²',assumptions:'“人为删项”不重拟合；“两项字典重拟合”与“三项字典重拟合”共享页面给出的 8 个训练状态。少放一项时，它的作用可能被剩余系数部分吸收；加入 x² 后，这个三角映射可以在完整字典内闭合。',chart:'留出初值下第二状态的滚动预测',reading:'Notebook 给出 EDMD 的矩阵拟合步骤，并比较字典失配与正则化。',first:['留出初值 x / U',.1,1.8,.1,1.5],second:['预测步数',1,12,1,8],kind:'drop',run:(a,b,k)=>lifted(a,b,k)},
 '14.4':{title:'阈值越高，方程就越真实吗？',question:'先预测：把候选项不断删去，何时会连已知真项也丢失？',context:'50 个训练状态 x∈[0.1,1] U 的候选项为 [1,x,x²]，教学真导数为 1.2x−0.4x²。先按训练列二范数归一化，再做顺序阈值回归；曲线显示到训练范围之外。',formula:'d≈Θξ；支持由归一化系数阈值决定',assumptions:'导数叠加固定波形扰动，调节幅度时保留波形，便于比较阈值筛选的变化。',chart:'候选导数曲线与已知生成方程',reading:'Notebook 给出归一化、迭代重拟合、导数噪声和独立轨迹核验。',first:['归一化系数阈值',.01,6,.01,.01],second:['导数扰动幅度 / U/T',0,.3,.01,0],kind:null,run:(a,b)=>sparseView(a,b)},
