...
const configs={
 ...
 '14.3':{title:'加入一个平方观测量，会改变预测吗？',question:'先预测：当下一步 y 含 x²，只有 [x,y] 的字典会遗漏什么？',context:'教学三角映射使用 x⁺=0.8x、y⁺=0.7y+0.4x²。短字典只含 [x,y]，完整字典再含 x²。',formula:'[x,y,x²]⁺ = K[x,y,x²]；短字典遗漏 x²',assumptions:'这里的完整字典专为该系统构造且恰好闭合；短字典显示同一对称训练范围下的简化传播，不代表所有 EDMD 拟合。',chart:'留出初值下第二状态的滚动预测',reading:'Notebook 展示观测函数、EDMD 回归、读出矩阵、字典失配与正则化。',first:['留出初值 x / U',.1,1.8,.1,1.5],second:['预测步数',1,12,1,8],kind:'short',run:(a,b,k)=>lifted(a,b,k)},
...
function draw(config,chapter){
 ...
 if(chapter==='14.3')$('toggle-case').textContent=state.kind==='short'?'切换到三项字典':'切换回短字典';
